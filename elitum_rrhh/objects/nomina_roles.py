...
class SalaryAdvanceEliterpCancel(models.Model):
    _name = 'elitumgroup.advanced.cancel'

    _description = u'Modelo - Razón para Negar Anticipo'

    description = fields.Text(u'Descripción', required=True)

    def advanced_cancel(self):
        anticipo_id = self.env['salary.advanced.eliterp'].browse(self._context['active_id'])
        anticipo_id.update({
            'state': 'negado',
            'negado': self.description
        })
        return anticipo_id
...
class SalaryAdvanceEliterp(models.Model):
    ...
    @api.multi
    def solicitar_aprobar(self):
        '''Solicitar Aprobación'''
        if len(self.line_salary_advanced) == 0:
            raise except_orm("Error", "No hay Empleados en Anticipo")
        self.update({'state': 'para_aprobar'})

    @api.multi
    def aprobar_anticipo(self):
        '''Aprobar Anticipo'''
        self.update({
            'state': 'aprobado',
            'user_aprobar': self._uid
        })

    def action_advanced_cancel(self):
        ''''Díalogo Negar Anticipo'''
        return {
            'name': "Explique la Razón",
            'view_mode': 'form',
            'view_type': 'form',
            'res_model': 'elitumgroup.advanced.cancel',
            'type': 'ir.actions.act_window',
            'target': 'new',
            'context': {},
        }
    ...
    state = fields.Selection([
        ('draft', 'Borrador'),
        ('para_aprobar', 'Para Aprobación'),
        ('aprobado', 'Aprobado'),
        ('open', 'Confirmado'),
        ('contabilizado', 'Contabilizado'),
        ('denied', 'Negado')], string="Estado", default='draft')
    user_aprobar = fields.Many2one('res.users', string='Usuario de Aprobación')
    negado = fields.Text('Negado por')
