...
# MARZ (Crear esté modelo y llenar con datos del biométrico [ELITUMDEVELOP])
class TypeRegisterAttendance(models.Model):
    _name = 'elitumgroup.hr.attendance.type'
    _description = 'Tipo de Registro (Asistencias)'

    code = fields.Integer('Código', required=True, size=2)
    name = fields.Char('Nombre', required=True)

    _sql_constraints = [
        ('rrhh_hr_attendance_type_code_unique', 'unique (code)', "El Código debe ser único")
    ]

class HrAttendance(models.Model):
    _inherit = "hr.attendance"
    _order = "datetime_check desc"

    check_in = fields.Datetime(string="Check In", default=fields.Datetime.now, required=False)
    datetime_check = fields.Datetime('Fecha/Hora', required=True)
    type_check = fields.Many2one('elitumgroup.hr.attendance.type', 'Tipo de Registro', required=True)
    notes = fields.Text()
    ...
    def name_get(self):
        result = []
        for data in self:
            result.append((data.id, "%s (%s) - %s" % (
                data.type_check.name,
                data.datetime_check,
                data.employee_id.name
            )))
        return result

    @api.constrains('datetime_check')
    def _check_validity_check_in_check_out(self):
        # Soló ELITUMDEVELOP
        return

    @api.constrains('datetime_check', 'employee_id')
