...
class Contract(models.Model):
    ...
    def activar_contrato(self):
        ...
        secuencial = len(self.env['hr.contract'].search([('state_eliterp', '!=', 'draft')]))
        new_name = new_name.split('-')
        secuencial = secuencial + 1
        secuencial_name = ''
        if secuencial < 10:
            secuencial_name = '00' + str(secuencial)
        elif secuencial >= 10 and secuencial < 100:
            secuencial_name = '0' + str(secuencial)
        else:
            secuencial_name = str(secuencial)
        name = new_name[0] + '-' + new_name[1] + '-' + new_name[2] + '-' + secuencial_name
        return self.write({
            'name': name,
