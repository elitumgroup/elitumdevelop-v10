...
class ReporteAsistencias(models.AbstractModel):
    _name = 'report.elitum_rrhh.reporte_asistencias_pdf'

    @api.model
    def render_html(self, docids, data=None):
        docargs = {
            'doc_ids': docids,
            'doc_model': 'elitumgroup.report.asistencias',
            'docs': self.env['elitumgroup.report.asistencias'].browse(docids),
            'data': data,
            'get_lines': self.env['elitumgroup.report.asistencias'].get_lines,
        }
        return self.env['report'].render('elitum_rrhh.reporte_asistencias_pdf', docargs)
