...
class ReporteEmpleados(models.TransientModel):
    ...
    def get_lines(self, context):
        ...
        if context['tipo_reporte'] != 'all':
            arg.append(('state_laboral', '=', 'activo'))
    ...
    tipo_reporte = fields.Selection([
        ('all', 'Todos'),
        ('activos', 'Activos')
    ], string="Tipo de Reporte", default='all')
...
class ReporteAsistencias(models.TransientModel):
    _name = 'elitumgroup.report.asistencias'

    _description = 'Reporte Asistencias'

    def date_range(self, start_date, end_date):
        """
        Devolvemos una lista de fechas en un rango
        """
        dates = []
        ...
        for n in range(int((end_date - start_date).days) + 1):
            ...
            day_week = new_date.strftime('%A')
            if not day_week in ['Saturday', 'Sunday']:
                dates.append(new_date)
        return dates

    def get_lines(self, context):
        dates = self.date_range(context['fecha_inicio'], context['fecha_fin'])
        if context['tipo_reporte'] != 'todos':
            if isinstance(context['empleados'], int):
                employees = context['empleados']
            else:
                id = context['empleados'].id
                employees = self.env['hr.employee'].browse(id)
        else:
            employees = self.env['hr.employee'].search([('state_laboral', '=', 'activo'), ('id_biometric', '!=', False)]).sorted(lambda x: x.apellidos)
        registros = []
        for employee in employees:
            sql = ""
            for date in dates:
                fecha_string = date.strftime('%Y-%m-%d')
                sql = """SELECT DISTINCT a.type_check, a.datetime_check - interval '5 hours' AS datetime_check \
                                  FROM hr_attendance AS a \
                                  INNER JOIN elitumgroup_hr_attendance_type AS b ON a.type_check = b.id
                                  WHERE CAST (a.datetime_check - interval '5 hours' AS DATE) = '%s' \
                                  AND a.employee_id = '%s' ORDER BY a.type_check ASC;""" % (
                fecha_string, employee.id)
                ...
                if response:
                    ...
                    a_retorno = False
                    salida = False
                    entrada_object = False
                    for line in response:
                        object_datetime = datetime.strptime(line['datetime_check'], '%Y-%m-%d %H:%M:%S')
                        if line['type_check'] == 1:
                            entrada = object_datetime.strftime('%H:%M:%S')
                            entrada_object = object_datetime
                        elif line['type_check'] == 2:
                            a_salida = object_datetime.strftime('%H:%M:%S')
                        elif line['type_check'] == 3:
                            a_retorno = object_datetime.strftime('%H:%M:%S')
                        else:
                            salida = object_datetime.strftime('%H:%M:%S')
                    hora_atraso = datetime(int(fecha_string[0:4]), int(fecha_string[5:7]), int(fecha_string[8:]), 9, 0, 0)
                    diferencia = 0
                    if entrada_object:
                        if entrada_object > hora_atraso: # Asegurarnos qué sean mayores a 09:00 AM
                            diferencia = (entrada_object - hora_atraso).seconds # Diferencia en segundos
                    tiempo = False
                    valor_tiempo = False
                    if diferencia > 300:
                        tiempo = True
                        valor_tiempo = str(timedelta(seconds=diferencia))
                    registros.append({
                        'name': employee.name,
                        'fecha': fecha_string,
                        'entrada': entrada if entrada else '-',
                        'a_salida': a_salida if a_salida else '-',
                        'a_retorno': a_retorno if a_retorno else '-',
                        'salida': salida if salida else '-',
                        'novedad': 'Atrasado' if tiempo else '-',
                        'tiempo': valor_tiempo if valor_tiempo else '-',
                        'comentario': '-',
                        'color': 'normal'
                    })
                else:
                    ausencias = self.env['hr.holidays'].search([('state', '=', 'validate'),
                                                                      ('date_from', '<=', fecha_string),
                                                                      ('date_to', '>=', fecha_string),
                                                                      ('employee_id', '=', employee.id)])
                    if not ausencias:
                        ausencias = self.env['hr.holidays'].search([('holiday_type', '=', 'category'),
                                                                    ('date_from', '<=', fecha_string),
                                                                    ('date_to', '>=', fecha_string)])
                    # No existen registros en el día
                    registros.append({
                        'name': employee.name,
                        'fecha': fecha_string,
                        'entrada': '-',
                        ...
                        'novedad': 'Ausencia',
                        'tiempo': '-',
                        'comentario': ausencias[0].name if ausencias else'Sin justificar',
                        'color': 'ausencia'
                    })
        return registros

    def imprimir_reporte_asistencias_pdf(self):
        return self.env['report'].get_action(self, 'elitum_rrhh.reporte_asistencias_pdf')

    tipo_reporte = fields.Selection([('todos', 'Todos'), ('individual', 'Empleado')], default='todos')
    empleados = fields.Many2one('hr.employee', domain=[('state_laboral', '=', 'activo'), ('id_biometric', '!=', False)])
    fecha_inicio = fields.Date('Fecha Inicio', required=True)
    fecha_fin = fields.Date('Fecha Fin', required=True)
