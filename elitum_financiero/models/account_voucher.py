...
class AccountVoucher(models.Model):
    ...
    def cargar_monto(self):
        ...
        if self.voucher_type == 'sale':
            ...
            self.cantidad = sum(line.monto_pago for line in self.lineas_pagos_proveedores)
            if self.forma_de_pago == 'cash' and self.beneficiario_proveedor == 'supplier':
                total = self.lineas_pagos_proveedores.filtered(lambda x: x.account_id == self.partner_id.property_account_payable_id).monto_pago
            else:
            ...
            for inv in self.lineas_cobros_facturas:
                if total == 0.00:
                    continue
                if inv.valor_aprobado_pago <= total:
    ...
    def onchange_banco_beneficiario_proveedor_forma_de_pago(self):
        ...
        if self.forma_de_pago <> 'cash':
            if self.beneficiario_proveedor == 'viaticos':
                self.account_id = self.env['account.account'].search([('code', '=', '1.1.2.3.9')])[0].id # Crear esa cuenta es necesario
    ...
    def imprimir_cheque(self):
        ...
        amount = self.env['report.elitum_contabilidad.reporte_factura_cliente'].get_amount_to_word(
            self.cantidad).upper()
        if self.banco.display_name == 'BANCO BOLIVARIANO (CTA. CTE. # 000-53XXXX-8)':
            result = {
