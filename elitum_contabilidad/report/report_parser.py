...
class ReporteLibroMayor(models.AbstractModel):
    ...
    def get_saldo_inicial(self, cuenta, fecha_inicio, fecha_fin):
        ...
        if cuenta.fecha_saldo_inicial >= fecha_inicio and cuenta.fecha_saldo_inicial <= fecha_fin:
...
class ReporteEstadoFinanciero(models.AbstractModel):
    ...
    def get_saldo_resultado(self, cuenta, tipo, fecha_inicio, fecha_fin):
        ...
        saldo_inicial = self.env['account.account'].search([
            ('id', '=', cuenta),
            ('fecha_saldo_inicial', '>=', fecha_inicio),
            ('fecha_saldo_inicial', '<=', fecha_fin)
        ])
        if saldo_inicial:
    ...
    def get_reporte(self, tipo, doc):
        ...
        if tipo == '3':
            ...
            if cuenta_estado['monto'] != 0.00:
                # MARZ
                monto = self.estado_resultado(doc.fecha_inicio, doc.fecha_fin)
