...
class AccountInvoice(models.Model):
    ...
    def nota_credito_create(self):
        ...
        description = ""
        nota_credito = self.with_context(nota_credito=True).refund(date, date, description,
                                                                   self.env['account.journal'].search(
                                                                       [('name', '=', 'Credito Contable')])[0].id)
        ...
        nota_credito.write({'invoice_id_ref': self.id, 'origin': self.numero_factura_interno})
    ...
    def imprimir_factura_cliente(self):
        ...
        result = {
            ...
            'report_name': 'elitum_ventas.reporte_factura_cliente_xls',
            'datas': {'ids': reporte},
            'context': {
                'reporte_factura_cliente': True
            }
        }
        return result
    ...
    def create(self, values):
        ...
        if flag_venta == True:
            mensaje = ""
            autorizacion = self.env['autorizacion.sri'].search([('tipo_comprobante', '=', 2), ('state', '=', 'activo')])
            ...
            if 'type' in values:
                if values['type'] == 'out_refund':
                    autorizacion = self.env['autorizacion.sri'].search(
                        [('tipo_comprobante', '=', 3), ('state', '=', 'activo')])
    ...
    numero_establecimiento = fields.Char(string=u'Número de Establecimiento', size=3, default='001')
    sustento_tributario = fields.Many2one('sustento.tributario', string=u'Sustento Tributario', required=True)
