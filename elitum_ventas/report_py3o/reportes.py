...
class ParserReVentas(models.TransientModel):
    ...
    def _extend_parser_context(self, context_instance, report_xml):
        ...
        if 'reporte_factura_cliente' in self._context:
            factura = self.env['account.invoice'].browse(context_instance.ids[0])
            lines = []
            for line in factura.invoice_line_ids:
                lines.append({
                    'cantidad': int(round(line.quantity)),
                    'desc': str(line.name),
                    'precio': format(line.price_unit, ',.2f'),
                    'total': format(round(float(line.quantity * line.price_unit), 2), ',.2f')
                })
            context_instance.localcontext.update({
                'fecha_nombre': self.env['eliterp.funciones'].get_date_format_invoice(factura.date_invoice),
                'total_cadena': self.env['eliterp.funciones'].get_amount_to_word(factura.amount_total),
                'base_cero_new': factura.base_cero_iva if int(factura.base_cero_iva) > 0 else '-',
                'base_iva_new': factura.amount_tax if int(factura.amount_tax) > 0 else '-',
                'iva': '12' if int(factura.amount_tax) > 0 else '0',
                'get_lines': lines
            })
