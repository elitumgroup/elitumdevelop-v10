...
class AccountInvoice(models.Model):
    ...
    payment_metod_ec = fields.Many2one('account.journal.payment.method', string='Forma de Pago', required=True)
    ...
    def _get_saldo_pedido(self):
        ...
        if self.pedido_relacionado:
            value = 0.00
            lines_pedido = sum(
                line.price_unit for line in self.pedido_relacionado.order_line.filtered(lambda x: x.facturado))
    ...
    def _onchange_pedido_relacionado(self):
        ...
        if self.pedido_relacionado:
            ...
            cuenta_anticipo = self.env['account.account'].browse(
                1292).id  # Predefinido, id = 1292 (ANTICIPOS DE CLIENTES)
            data.append([
                0,
                0,
                {
                    ...
                    'price_unit': round(self.pedido_relacionado.amount_untaxed - value_anteriores,
                                        2) if self.porcentaje_anticipo == 'total' else self._get_amount_anticipo(
                        self.pedido_relacionado),
                    'quantity': 1.0,
                    'discount': 0.0,
    ...
    # MARZ **
    @api.multi
    def sent_invoice_customer(self):
        '''Enviar Factura de Cliente por correo electrónico'''
        self.ensure_one()
        ir_model_data = self.env['ir.model.data']
        try:
            template_id = \
                ir_model_data.get_object_reference('elitum_ventas', 'eliterp_template_out_invoice')[1]
        except ValueError:
            template_id = False
        try:
